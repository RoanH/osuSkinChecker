...
public class SkinChecker{
	...
	private static void checkFiles(Path skinFolder, List<Path> foreign) throws IOException{
		//get skin size
		Queue<Path> files = new LinkedList<Path>();
		files.offer(skinFolder);
		int count = 0;
		while(!files.isEmpty()){
			Path file = files.poll();
			if(Files.isDirectory(file)){
				Files.newDirectoryStream(file).forEach(files::offer);
			}else{
				count++;
			}
		}
	 ...
	 */
	private static void checkAllFiles(Path dir, Deque<String> path, List<Path> foreign, Consumer<String> next) throws IOException{
		try(DirectoryStream<Path> files = Files.newDirectoryStream(dir)){
			for(Path f : files){
				if(Files.isDirectory(f)){
					...
					next.accept(f.getFileName().toString());
